...
class SpendingLedgerIntegrationTest extends FullContextIntegrationTest {
    ...
    void shouldRecordAndQuerySpendingSummary() {
        ...
        spendingLedgerService.recordSpending(
                agentId, UUID.randomUUID(), new BigDecimal("50.000000"), SOME_RECIPIENT, executedAt);
    ...
    void shouldReturnCorrectRollingWindowTotals() {
        ...
        spendingLedgerService.recordSpending(
                agentId, UUID.randomUUID(), new BigDecimal("10.000000"), SOME_RECIPIENT, within24Hours);
        spendingLedgerService.recordSpending(
                agentId, UUID.randomUUID(), new BigDecimal("20.000000"), SOME_RECIPIENT, within7Days);
        spendingLedgerService.recordSpending(
                agentId, UUID.randomUUID(), new BigDecimal("30.000000"), SOME_RECIPIENT, within30Days);
        spendingLedgerService.recordSpending(
                agentId, UUID.randomUUID(), new BigDecimal("40.000000"), SOME_RECIPIENT, olderThan30Days);
    ...
    void shouldHandleDuplicatePaymentIdGracefully() {
        ...
        var executedAt = Instant.now().minus(1, ChronoUnit.HOURS).truncatedTo(ChronoUnit.MICROS);
        var first = spendingLedgerService.recordSpending(
                agentId, paymentId, new BigDecimal("50.000000"), SOME_RECIPIENT, executedAt);

        // when
        var second = spendingLedgerService.recordSpending(
                agentId, paymentId, new BigDecimal("999.000000"), SOME_RECIPIENT, executedAt);
    ...
    void shouldNoOpDuplicatePaymentIdAcrossSeparateTransactions() {
        ...
        SpendingLedgerEntry first = transactionTemplate.execute(status -> spendingLedgerService.recordSpending(
                agentId, paymentId, new BigDecimal("50.000000"), SOME_RECIPIENT, executedAt));
        SpendingLedgerEntry second = transactionTemplate.execute(status -> spendingLedgerService.recordSpending(
                agentId, paymentId, new BigDecimal("999.000000"), SOME_RECIPIENT, executedAt));
        ...
        try {
            ...
            var summary = transactionTemplate.execute(status -> spendingLedgerService.getSpendingSummary(agentId, 120));
            var expectedSummary = SpendingSummary.builder()
                    .dailyTotal(new BigDecimal("50.000000"))
                    .weeklyTotal(new BigDecimal("50.000000"))
                    .monthlyTotal(new BigDecimal("50.000000"))
            ...
            transactionTemplate.executeWithoutResult(
                    status -> jdbcTemplate.update("DELETE FROM spending_ledger WHERE payment_id = ?", paymentId));
        }
    }
}
