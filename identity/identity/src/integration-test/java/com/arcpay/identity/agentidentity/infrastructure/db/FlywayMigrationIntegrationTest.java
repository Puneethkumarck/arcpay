...
class FlywayMigrationIntegrationTest extends FullContextIntegrationTest {
    ...
    void shouldCreateOwnersUniqueIndexes() {
        ...
        assertThat(indexDefs)
                .anySatisfy(def -> assertThat(def)
                        .containsIgnoringCase("UNIQUE")
                        .containsIgnoringCase("lower")
                        .containsIgnoringCase("wallet_address"));
    ...
    void shouldCreateAgentsUniqueIndexOnOwnerAndName() {
        // given / when
        var indexDefs = jdbcTemplate.queryForList(
                "SELECT indexdef FROM pg_indexes WHERE tablename = 'agents' ORDER BY indexname", String.class);
        ...
        assertThat(indexDefs)
                .anySatisfy(def -> assertThat(def).contains("owner_id").doesNotContainIgnoringCase("UNIQUE"));
