...
class FeignApiKeyResolverTest {
    ...
    void shouldResolveApiKeyAndPropagateAuthority() {
        // given
        given(identityClient.resolveApiKey(SOME_API_KEY_HASH)).willReturn(Optional.of(SOME_OFFICER_PRINCIPAL_RESPONSE));
    ...
    void shouldReturnEmptyWhenClientResolvesToNoOwner() {
        // given
        given(identityClient.resolveApiKey(SOME_API_KEY_HASH)).willReturn(Optional.empty());
