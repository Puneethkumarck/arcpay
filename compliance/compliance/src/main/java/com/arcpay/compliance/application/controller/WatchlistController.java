...
public class WatchlistController {
    ...
    public ResponseEntity<WatchlistEntryResponse> addAddress(
            @AuthenticationPrincipal OwnerPrincipal principal, @Valid @RequestBody WatchlistEntryRequest request) {
        ...
        return ResponseEntity.status(CREATED)
                .body(WatchlistEntryResponse.builder()
                        .address(normalized)
                        .label(request.label())
                        .build());
    }
    ...
    public ResponseEntity<Void> removeAddress(
            @AuthenticationPrincipal OwnerPrincipal principal, @PathVariable String address) {
    ...
    public List<WatchlistEntryResponse> listAddresses() {
        return watchlistStore.getAllAddresses().stream()
                .sorted()
                .map(address ->
                        WatchlistEntryResponse.builder().address(address).build())
                .toList();
    }
